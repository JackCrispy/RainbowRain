...
class Owner:
    ...
    async def load(self, *, cog_name: str):
        ...
        try:
            ...
            await self.bot.say("That command could not be found.")
        except CogLoadError as e:
            log.exception(e)
            traceback.print_exc()
            await self.bot.say("There was an issue loading the command. Check"
                               " your logs for more information.")
        except Exception as e:
            log.exception(e)
            traceback.print_exc()
            await self.bot.say('Command was found and possibly loaded but '
    ...
    async def unload(self, *, cog_name: str):
        ...
        if not self._does_cogfile_exist(module):
            await self.bot.say("That command file doesn't exist. I will not"
        ...
        try:  # No matter what we should try to unload it
            ...
            log.exception(e)
            traceback.print_exc()
            await self.bot.say('Unable to safely unload that command.')
        else:
            await self.bot.say("The command/cc has been unloaded.")
    ...
    async def unload_all(self):
        ...
        if still_loaded:
            ...
            await self.bot.say("I was unable to unload some commands: "
                "{}".format(still_loaded))
        else:
            await self.bot.say("All commands are now unloaded.")
    ...
    async def _reload(self, *, cog_name: str):
        ...
        try:
            ...
            await self.bot.say("That command cannot be found.")
        except NoSetupError:
            await self.bot.say("That command does not have a setup function.")
        except CogLoadError as e:
            log.exception(e)
            traceback.print_exc()
            await self.bot.say("That command could not be loaded. Check your"
                               " console or logs for more information.")
        else:
            set_cog(module, True)
            await self.disable_commands()
            await self.bot.say("The command has been reloaded.")
    ...
    async def bug(self, ctx, *, message : str):
        ...
        if self.bot.settings.owner is None:
            await self.bot.say("Error: none set")
            return
        server = ctx.message.server
    ...
    async def info(self):
        ...
        author_repo = "https://google.com"
        red_repo = author_repo + "https://google.com"
        server_url = "How to"
        dpy_repo = "https://github.com/Rapptz/discord.py"
        python_url = "https://www.python.org/"
        since = datetime.datetime(2018, 3, 6, 0, 0)
        ...
        about = (
            "Galactic Rift \n"
            "Hello!, you are using Galactic Rift v1.0 \n\n\n"
            "To report a user use the command \n\n"
            ">report (user) (reason) \n"
            ">bug (bug)\n\n"
            "".format(red_repo, author_repo, server_url))

        embed = discord.Embed(colour=discord.Colour.red())
        embed.add_field(name="Creator", value=str(owner))
        embed.add_field(name="Python", value=py_version)
        embed.add_field(name="discord.py", value=dpy_version)
        embed.add_field(name="Infomation", value=about, inline=False)
        embed.set_footer(text="Bringing joy since 3/06/2018 (over "
